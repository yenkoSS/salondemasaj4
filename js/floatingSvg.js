// images instead of bubbles
/* */
const toothImg = new Image();
toothImg.src = "./icons/happy.svg"; //
...
(() => {
  ...
  function initBubbleBackground(section, cfg = CONFIG) {
    ...
    function spawnBubbles() {
      ...
      const spinMin = ((cfg.spinMinDeg ?? -12) * Math.PI) / 180;
      const spinMax = ((cfg.spinMaxDeg ?? 12) * Math.PI) / 180;
      ...
      for (let i = 0; i < target; i++) {
        ...
        state.bubbles.push({
          ...
          rotationSpeed: rand(spinMin, spinMax),
        });
      }
    }
    ...
    function drawBubble(b) {
      ...
      ctx.drawImage(toothImg, -w / 2, -h / 2, w, h);
      ctx.globalAlpha = 1;
      ctx.restore();
    }
